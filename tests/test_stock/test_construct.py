...
def test_add():
    ...
    new = a + b
    assert isinstance(new, sf.operation.AddOperation)
    assert new() == 2 + 8
    assert a.value == 2
    assert b.value == 8
...
def test_sub():
    ...
    new = a - b
    assert isinstance(new, sf.operation.SubOperation)
    assert new() == 2 - 8
    assert a.value == 2
    assert b.value == 8
...
def test_multiply():
    ...
    new = a * b
    assert isinstance(new, sf.operation.MulOperation)
    assert new() == 2 * 8
    assert a.value == 2
    assert b.value == 8
...
def test_divide():
    ...
    new = a / b
    assert isinstance(new, sf.operation.DivOperation)
    assert new() == 2 / 8
    assert a.value == 2
    assert b.value == 8
